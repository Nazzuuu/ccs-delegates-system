...
const transferFilterPresetsHelp = transferDataTypes.map((type)=>`${type} (${TRANSFER_FILTER_PRESET_DESCRIPTIONS[type]})`).join('; ');
...
const excludeOption = new Option('--exclude <comma-separated data types>', `Exclude data: ${transferFilterPresetsHelp}`).argParser(getParseListWithChoices(transferDataTypes, 'Invalid options for "exclude"'));
const onlyOption = new Option('--only <command-separated data types>', `Include only these types (plus schemas): ${transferFilterPresetsHelp}`).argParser(getParseListWithChoices(transferDataTypes, 'Invalid options for "only"'));
...
export { abortTransfer, buildTransferTable, createStrapiInstance, excludeOption, exitMessageText, formatDiagnostic, getAssetsBackupHandler, getDefaultExportName, getDiffHandler, getTransferTelemetryPayload, isIgnoredContentType, loadersFactory, logTransferFilterSummary, onlyOption, parseRestoreFromOptions, setSignalHandler, shouldSkipStage, throttleOption, validateExcludeOnly };
