...
function openEdit(d: { id: number; studentId: string; name: string; yearLevel: string }) {
  editId.value        = d.id
  editStudentId.value = d.studentId ?? ''
...
<template>
  <div class="p-3 sm:p-6">
            ...
            <tr v-for="(d, idx) in paginated" :key="d.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors">
              ...
              <td class="table-td text-center">
                <div class="flex items-center justify-center gap-1.5">
                  <button @click="openEdit(d)" class="btn-secondary inline-flex items-center gap-1 text-xs px-2.5 py-1">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536M9 11l6-6 3 3-6 6H9v-3z"/><path d="M4 20h4l10-10a2.828 2.828 0 00-4-4L4 16v4z"/></svg>
                    Edit
                  </button>
    ...
    <!-- Add student modal -->
    <Teleport to="body">      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    ...
    <Teleport to="body">
      <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 border border-gray-200 dark:border-gray-700">
          <h3 class="text-lg font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
            <svg class="w-5 h-5 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536M9 11l6-6 3 3-6 6H9v-3z"/><path d="M4 20h4l10-10a2.828 2.828 0 00-4-4L4 16v4z"/></svg>
            Edit Student
          </h3>
          <div class="space-y-3">
            <div>
              <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Student ID</label>
              <input v-model="editStudentId" type="text" placeholder="e.g. 4261124" class="input-search" @keyup.enter="handleEditSave"/>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Full Name</label>
              <input v-model="editName" type="text" placeholder="e.g. DELA CRUZ, JUAN" class="input-search" @keyup.enter="handleEditSave"/>
              <p v-if="editError" class="text-red-500 text-xs mt-1">{{ editError }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Year Level</label>
              <select v-model="editYear" class="input-search">
                <option v-for="y in yearLevels.filter(y=>y!=='All')" :key="y" :value="y">{{ shortYear(y) }}</option>
              </select>
            </div>
            <!-- Sync status -->
